...
DOCS_DIR = "docs"
MAIN_PAGE_PATH = os.path.join(DOCS_DIR, "README.md")
PROJECTS_PAGE_PATH = os.path.join(DOCS_DIR, "Projects.md")
...
def generate_wiki_article(repo):
    ...
    prompt = f"""
Create a Wikipedia-style article in Markdown format for the following GitHub repository:
Repo Name: {name}
...
Formatting Rules:
- Use ## for level-2 headers and ### for level-3 headers.
- Use **Bold** for key terms and *Italic* for emphasis.
- Use [Link Text](URL) for external links.
- Use [[Page Name]] style internal links as plain text references (e.g. [Page Name](Page-Name.md)).
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Wikipedia articles in Markdown format."
                ),
            },
...
def main():
    ...
    for repo in repos:
        ...
        if os.path.exists(filepath):
            ...
            repo_links.append(f"- [{name}]({filename})")
            continue
    ...
    if repo_links:
        if os.path.exists(PROJECTS_PAGE_PATH):
        ...
        if not any("Browse all project repositories" in line for line in main_lines):
            ...
            project_link_msg = "- [Browse all project repositories](Projects.md)\n"
            if nav_index != -1:
                main_lines.insert(nav_index + 1, "\n")
                main_lines.insert(nav_index + 2, project_link_msg)
            else:
                main_lines.append("\n" + project_link_msg)
