...
## Installation
To install youtube-dl, follow these steps:
* For **UNIX** users (Linux, macOS, etc.): Use `sudo curl -L https://yt-dl.org/downloads/latest/youtube-dl -o /usr/local/bin/youtube-dl` or `sudo wget https://yt-dl.org/downloads/latest/youtube-dl -O /usr/local/bin/youtube-dl`
* For **Windows** users: Download the [youtube-dl.exe](https://yt-dl.org/latest/youtube-dl.exe) file and add it to the system **PATH**
* For **macOS** users: Install using [Homebrew](https://brew.sh/) with `brew install youtube-dl` or [MacPorts](https://www.macports.org/) with `sudo port install youtube-dl`
